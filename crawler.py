...
def google_image_url_from_webpage(driver, max_number, quiet=False):
    ...
    while True:
        try:
            thumb_elements = driver.find_elements(By.CLASS_NAME, "ob5Hkd")
    ...
    for i, elem in enumerate(thumb_elements):
        try:
            if i > max_number:
                break
            cur_elem = elem.find_elements(By.TAG_NAME, "a")[0]
            if cur_elem.is_displayed() and cur_elem.is_enabled():
                cur_elem.click()
            # image_elements = driver.find_elements(By.CLASS_NAME, "islib")
            time.sleep(random.randint(1, 5))
            image_elements = driver.find_elements(By.XPATH, "//img[@jsname = 'kn3ccd']")
            if len(image_elements) != 0:
                image_urls.append(image_elements[0].get_attribute("src"))
                my_print(f"add {i+1} image url ...")
        except:
            my_print("被ban啦，快去操作下~")
            time.sleep(10)
            error += 1
            if error > 3:
                break
